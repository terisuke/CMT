...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  if (companyError || !company)
    return json({ error: "企業情報の取得に失敗しました", company: null, transactions: [] });
  const { data: transactions, error } = await getTransactionsByCompanyId(request, companyId);
  if (error)
    return json({ error: "取引データの取得に失敗しました", company, transactions: [] });
  return json({ company, transactions: transactions || [], error: null });
}
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (actionType === "delete") {
    ...
    if (!transactionId || typeof transactionId !== "string")
      return json({ success: false, error: "取引IDが無効です" });
    const { error } = await deleteTransaction(request, transactionId);
    if (error)
      return json({ success: false, error: "取引の削除に失敗しました" });
    return json({ success: true, error: null });
  }
  return json({ success: false, error: "不明なアクションタイプです" });
}
...
export default function TransactionsList() {
  ...
  // URL セグメント数が多い場合は子ルート（詳細・編集・新規作成）を Outlet 経由で表示
  const segments = location.pathname.split("/");
  if (segments.length > 4) return <Outlet context={{ company }} />;
                ...
                {filteredTransactions.map((transaction: Transaction) => (
                  <tr key={transaction.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      ...
                      <button onClick={() => openDeleteModal(transaction)} className="text-red-600 hover:text-red-900">
                        削除
                      </button>
                    </td>
                  </tr>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full p-6 shadow-xl">
            ...
            <p className="text-sm text-gray-500 mb-4">本当にこの取引を削除しますか？この操作は元に戻せません。</p>
